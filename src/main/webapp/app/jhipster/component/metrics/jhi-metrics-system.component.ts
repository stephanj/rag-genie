...
export class JhiMetricsSystemComponent {
     ...
     */
    @Input() systemMetrics!: {[key: string]: number};
    ...
    convertMillisecondsToDuration(ms: number): string {
        ...
        for (const key in times) {
            if (Math.floor(ms / (times as any)[key]) > 0) {
                let plural = '';
                if (Math.floor(ms / (times as any)[key]) > 1) {
                    plural = 's';
                }
                timeString += Math.floor(ms / (times as any)[key]).toString() + ' ' + key.toString() + plural + ' ';
                ms = ms - (times as any)[key] * Math.floor(ms / (times as any)[key]);
            }
        }
        return timeString;
    }
