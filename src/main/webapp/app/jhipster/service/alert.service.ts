...
export class JhiAlertService {
    ...
    constructor(
        ...
        this.toast = config.alertAsToast || false;
        this.i18nEnabled = config.i18nEnabled || false;
        this.alertId = 0; // unique id for each alert. Starts from 0.
        this.alerts = [];
        this.timeout = config.alertTimeout || 0;
    }
    ...
    addAlert(alertOptions: JhiAlert, extAlerts: JhiAlert[]): JhiAlert {
        ...
        if (alertOptions.timeout && alertOptions.timeout > 0) {
             ...
             this.ngZone.runOutsideAngular(() => {
                  setTimeout(() => {
                      this.ngZone.run(() => {
                          this.closeAlert(alertOptions.id || 1, extAlerts);
                      });
    ...
    private factory(alertOptions: JhiAlert): JhiAlert {
        const alert: JhiAlert = {
            ...
            close: (alerts: JhiAlert[]) => {
                return this.closeAlert(alertOptions.id || 1, alerts);
            }
        };
        if (!alert.scoped) {
